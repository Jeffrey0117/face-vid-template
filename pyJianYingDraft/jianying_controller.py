...
class ControlFinder:
    ...
    def class_name_matcher(class_name: str, depth: int = 1, exact: bool = False) -> Callable[[uia.Control, int], bool]:
        ...
        def matcher(control: uia.Control, _depth: int) -> bool:
            ...
            def _list_available_drafts(self) -> List[str]:
                """列出所有可用的草稿名稱（用於診斷）"""
                try:
                    # 尋找所有包含HomePageDraftTitle的控件
                    draft_controls = self.app.TextControl(
                        searchDepth=3,
                        Compare=ControlFinder.desc_matcher("HomePageDraftTitle", exact=False)
                    )
        
                    available_drafts = []
                    if hasattr(draft_controls, 'GetChildren') and draft_controls.Exists(0):
                        children = draft_controls.GetChildren()
                        for child in children:
                            try:
                                desc = child.GetPropertyValue(30159) or ""
                                if "HomePageDraftTitle:" in desc:
                                    draft_name = desc.split("HomePageDraftTitle:", 1)[1]
                                    if draft_name:
                                        available_drafts.append(draft_name.strip())
                            except:
                                continue
        
                    # 如果沒有找到，嘗試更廣泛的搜索
                    if not available_drafts:
                        all_desc_controls = self.app.FindAll(ControlFinder.desc_matcher("HomePageDraftTitle", exact=False))
                        for control in (all_desc_controls or []):
                            try:
                                desc = control.GetPropertyValue(30159) or ""
                                if ":" in desc:
                                    draft_name = desc.split(":", 1)[1]
                                    available_drafts.append(draft_name.strip())
                            except:
                                continue
        
                    return list(set(available_drafts))  # 去除重複
        
                except Exception as e:
                    return [f"列出草稿時發生錯誤: {str(e)}"]
    ...
    def export_draft(self, draft_name: str, output_path: Optional[str] = None, *,
        ...
        **注意: 需要确认有导出草稿的权限(不使用VIP功能或已开通VIP), 否则可能陷入死循环**
        ...
        if not draft_name_text.Exists(0):
            ...
            draft_name_text = self.app.TextControl(
                searchDepth=2,
                Compare=ControlFinder.desc_matcher(f"HomePageDraftTitle:{draft_name}", exact=False)
            )
            if not draft_name_text.Exists(0):
                # 列出所有可用的草稿以供參考
                available_drafts = self._list_available_drafts()
                error_msg = f"未找到名为'{draft_name}'的剪映草稿\n"
                error_msg += f"可用的草稿: {available_drafts[:10]}"  # 只顯示前10個避免訊息過長
                if len(available_drafts) > 10:
                    error_msg += f"...等共{len(available_drafts)}個"
                raise exceptions.DraftNotFound(error_msg)
